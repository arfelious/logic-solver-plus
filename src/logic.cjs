...
Logic._assert = function (value, tester, description) {
  if (! tester(value)) {
    var displayValue = (typeof value === 'string' ||1? JSON.stringify(value) :
                        value);
...
Logic.Clause.prototype.append = function (/*formulaOrArray, ...*/) {
  return new Logic.Clause(this.terms.concat(Array.prototype.concat.apply([], arguments)));
};
...
Logic.Termifier.prototype.clause = function (/*args*/) {
  ...
  return new Logic.Clause(formulas.map(e=>self.term(e)));
};
...
Logic.Solver = class Solver{
    hasInitialized = false;
    clauses = []; // mutable [Clause]
    _num2name = [null]; // no 0th var
    _name2num = {}; // (' '+vname) -> vnum
    _minisat = null;
    _formulaInfo = {}; // Formula guid -> FormulaInfo
    _nextFormulaNumByType = {}; // Formula type -> next var id
    _ungeneratedFormulas = {}; // varNum -> Formula
    _F_used = false;
    _T_used = false;
    ...
    constructor(){
      this._termifier = new Logic.Termifier(this);
        // true and false
        if(!this.getVarNum){
            //he did not do new Logic.Solver
            throw new Error("Logic.Solver must be invoked using new.")
        }
        var F = this.getVarNum(Logic.NAME_FALSE, false, true); // 1
        var T = this.getVarNum(Logic.NAME_TRUE, false, true); // 2
        ...
        this.clauses.push(new Logic.Clause(-Logic.NUM_FALSE));
        this.clauses.push(new Logic.Clause(Logic.NUM_TRUE));
    }
}
...
Logic.Solver.prototype._useFormulaTerm = function (t, _addClausesOverride) {
  ...
  if (! self._ungeneratedFormulas.hasOwnProperty(v)) {
    return;
  }
...
//Thanks to https://github.com/meteor/logic-solver/issues/5#issuecomment-540536668
...
Logic.product = function (/*formulaOrBitsOrArray, ...*/) {
  var things = Array.prototype.concat.apply([], arguments);
  if (assert) assert(things, isArrayWhere(isFormulaOrTermOrBits));
...
Logic.subtract = function (minuend, subtrahend) {
  ...
  if (minuend instanceof Logic.Bits) {
    minuend.bits.forEach(function (b, i) {
      pushToNth(binaryWeightedMinuend, i, b);
    });
  } else {
    pushToNth(binaryWeightedMinuend, 0, minuend);
  }
  var binaryWeightedSubtrahend = [];
  if (subtrahend instanceof Logic.Bits) {
    subtrahend.bits.forEach(function (b, i) {
      pushToNth(binaryWeightedSubtrahend, i, b);
    });
  } else {
  ...
  if (binaryWeightedMinuend.length < binaryWeightedSubtrahend.length) {
    return new Logic.Bits(Array(binaryWeightedSubtrahend.length).fill(Logic.FALSE));
  }

  var output = [];
  var borrow = Logic.FALSE;

  for (var i = 0; i < binaryWeightedMinuend.length; i++) {
    var a = (binaryWeightedMinuend[i] || Logic.FALSE);
    var b = (binaryWeightedSubtrahend[i] || Logic.FALSE);
    var diff = Logic.xor(Logic.xor(a, b), borrow);
    var newBorrow = Logic.or(
      Logic.and(Logic.not(Logic.or(a,Logic.FALSE)), b),
      Logic.and(Logic.not(Logic.xor(a, b)), borrow)
    );

    output.push(diff);
    borrow = newBorrow;
  }
  return new Logic.Bits(output);
};
////////////////////////////////////////
...
Logic.Solution = function (_solver, _assignment) {
  ...
  self._ungeneratedFormulas = clone(_solver._ungeneratedFormulas);
  ...
  if (formula instanceof Logic.NotFormula) {
    ...
    if (typeof cachedResult === 'boolean') {
      ...
      if (info && info.varNum && info.varNum < assignment.length &&
          ! self._ungeneratedFormulas.hasOwnProperty(info.varNum)) {
        ...
        var value = clauses.every(function (cls) {
          return cls.terms.some(t=>self.evaluate(t));
        });
      }
